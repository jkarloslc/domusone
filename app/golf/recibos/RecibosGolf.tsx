...
// embedded: sin breadcrumb/header propio (para usarse como tab de otra página)
// soloMembresias: solo recibos con al menos una cuota que NO sea pensión de carrito
type Props = { embedded?: boolean; soloMembresias?: boolean }

export default function RecibosGolf({ embedded = false, soloMembresias = false }: Props) {
  ...
  const cargar = useCallback(async () => {
    ...
    const { data } = await q
    let rows = (data as unknown as Recibo[]) ?? []
    if (soloMembresias) rows = rows.filter(r => r.recibos_golf_det.some(d => d.tipo !== 'PENSION_CARRITO'))
    setRecibos(rows)
    setLoading(false)
    setPagina(1)
  }, [filtroStatus, soloMembresias])
