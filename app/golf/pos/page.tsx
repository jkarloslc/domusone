...
export default function POSPage() {
  ...
  // ── Cancelar venta ────────────────────────────────────────
  // Único método soportado para revertir cuotas es el de "Cancelar" en la
  // pantalla de Recibos de cada módulo (Golf/Hípico/Locales/Residencial) —
  // ahí sí se puede cancelar aunque el recibo nunca haya generado ticket POS
  // (generar el ticket es un paso opcional y posterior al recibo). Si esta
  // venta viene de uno de esos recibos, se bloquea aquí y se manda al origen
  // en vez de reimplementar la cascada; si es una venta directa de POS
  // (Green Fee, Pro Shop, Torneos, etc.) no hay cuota que revertir y se
  // cancela igual que siempre.
  const cancelarVenta = async (id: number) => {
    try {
      const [{ data: recGolf }, { data: recHip }, { data: recLoc }, { data: recResi }] = await Promise.all([
        dbGolf.from('recibos_golf').select('folio').eq('id_venta_pos_fk', id).maybeSingle(),
        dbHip.from('recibos_hip').select('folio').eq('id_venta_pos_fk', id).maybeSingle(),
        dbCtrl.from('loc_recibos').select('folio').eq('id_venta_pos_fk', id).maybeSingle(),
        dbCtrl.from('recibos').select('folio').eq('id_venta_pos_fk', id).maybeSingle(),
      ])
