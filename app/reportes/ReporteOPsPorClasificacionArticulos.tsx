...
// Fila sintética: la OP repartida entre las categorías de los artículos de
...
export default function ReporteOPsPorClasificacionArticulos() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: ccs }, { data: ars }, { data: ps }, { data: opsData }, { data: junction }] = await Promise.all([
      ...
      dbComp.from('ordenes_pago')
        .select('id, folio, concepto, monto, saldo, fecha_op, fecha_vencimiento, status, id_proveedor_fk, id_centro_costo_fk, id_area_fk, id_oc_fk')
    ...
    // OPs "con OC": FK directa en el encabezado o fila en la tabla puente ordenes_pago_oc
    const idsConOC = new Set<number>((junction ?? []).map((j: any) => j.id_op_fk))
    const conOC = ((opsData ?? []) as OP[]).filter(op => op.id_oc_fk != null || idsConOC.has(op.id))
    setOpsConOC(conOC)

    const catsPorOp = await resolverCategoriasPorOp(conOC.map(o => ({ id: o.id, id_oc_fk: o.id_oc_fk })))
  ...
  }, [opsCat, filtroStatus, filtroCC, filtroArea, filtroProv, filtroCategoria, filtroDe, filtroA])

  // Agrupar por Categoría (clasificación de artículo)
  ...
  // Matriz Categoría × Centro de Costo
  ...
  const toggleCat    = (key: string) => setExpandedCat(prev => {
    const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n
  })
  const expandAll   = () => setExpandedCat(new Set(grupos.map(g => g.nombre)))
  const collapseAll = () => setExpandedCat(new Set())

  // KPIs por status: sobre las OPs con OC originales (sin explotar por
  // categoría), para no duplicar el conteo de una OP que reparte su monto
  // entre varias categorías. Ignora el filtro de status, respeta el resto.
  const opsParaKPIs = useMemo(() => {
    return opsConOC.filter(op => {
      if (filtroCC   && op.id_centro_costo_fk !== Number(filtroCC)) return false
      if (filtroArea && op.id_area_fk !== Number(filtroArea)) return false
      if (filtroProv && op.id_proveedor_fk !== Number(filtroProv)) return false
      if (filtroDe   && (!op.fecha_op || op.fecha_op < filtroDe)) return false
  ...
  }, [opsConOC, filtroCC, filtroArea, filtroProv, filtroDe, filtroA])
  ...
  const exportExcel = () => {
    ...
    ws1['!cols'] = [{ wch: 32 }, { wch: 8 }, { wch: 14 }, { wch: 14 }, { wch: 14 }]
    ...
    ws2['!cols'] = [
      { wch: 14 }, { wch: 30 }, { wch: 10 }, { wch: 22 }, { wch: 20 }, { wch: 24 }, { wch: 32 },
      { wch: 12 }, { wch: 12 }, { wch: 13 }, { wch: 13 }, { wch: 13 }, { wch: 14 },
    ]
    XLSX.utils.book_append_sheet(wb, ws2, 'Detalle')

    const header: any[] = ['Categoría', ...matriz.columnas.map(([, v]) => v.nombre), 'TOTAL']
    ...
    ws3['!cols'] = [{ wch: 30 }, ...matriz.columnas.map(() => ({ wch: 14 })), { wch: 14 }]
    XLSX.utils.book_append_sheet(wb, ws3, 'Matriz Categoria x CC')

    const today = new Date().toISOString().slice(0, 10)
    XLSX.writeFile(wb, `OPs-por-Clasificacion-Articulos_${today}.xlsx`)
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select className="select" style={{ minWidth: 220 }} value={filtroCategoria} onChange={e => setFiltroCategoria(e.target.value)}>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 12, borderBottom: '1px solid #e2e8f0' }}>
        ...
        >
          <LayoutList size={14} /> Jerárquico (Categoría → OPs)
        </button>
        <button
        ...
        >
          <Grid3x3 size={14} /> Matriz Categoría × CC
        </button>
      </div>

      <PrintBar title="OPs-por-Clasificacion-Articulos" count={docsTotal} reportTitle="Órdenes de Pago por Clasificación de Artículos (con OC)" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          { label: 'Total Órdenes',   value: String(opsParaKPIs.length), color: '#7c3aed' },
        ].map(k => (
      ...
      <div id="reporte-print-area">
        {loading ? (
          ...
          <table id="reporte-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                <th style={{ textAlign: 'left' }}>Folio / Categoría</th>
            ...
            <tbody>
              {grupos.map(g => {
                const open = expandedCat.has(g.nombre)
                return (
                  <Fragment key={`cat-frag-${g.nombre}`}>
                    <tr style={{ background: '#eff6ff', cursor: 'pointer' }} onClick={() => toggleCat(g.nombre)}>
                    ...
                    {open && g.ops.map(op => {
                      ...
                      return (
                        <tr key={`op-${op.id}-${op._catIdx}`}>
                          ...
                          <td style={{ textAlign: 'right', fontSize: 11, color: 'var(--text-muted)' }}>{Math.round(op.fraction * 100)}%</td>
          ...
          <div className="card" style={{ padding: 0, overflow: 'auto' }}>
            <table id="reporte-table" style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
              <thead>
                <tr>
                  <th style={{ textAlign: 'left', position: 'sticky', left: 0, background: '#f1f5f9', zIndex: 2 }}>Categoría</th>
