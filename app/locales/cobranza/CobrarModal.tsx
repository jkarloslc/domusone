    ...
    try {
      ...
      if (!ventaId) {
        ...
        const { data: venta, error: ev } = await dbGolf.from('ctrl_ventas').insert({
          ...
          status: 'PAGADA', facturable: rf.facturable ?? false, usuario_crea: authUser?.user?.email ?? 'locales',
          notas: `Ticket POS desde recibo de locales ${exito.folio}`,
      ...
      const ticketData = {
        ...
        cliente: nombreArrendatario, cajero: authUser?.user?.email ?? 'locales', centro: centroLoc.nombre,
