...
export default function EventosPage() {
  ...
  const [gastoNotas,   setGastoNotas]   = useState<Record<number, boolean>>({}) // expanded notas
  ...
  const loadGastos = useCallback(async (evtId: number) => {
    ...
    setGastos(rows)
    // Load OP folios for gastos already linked to an OP
...
.meta { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 18px; padding: 14px 16px; background: #faf5ff; border-radius: 8px; border: 1px solid #e9d5ff; }
.meta .f label { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; color: #9333ea; display: block; margin-bottom: 2px; }
.meta .f span { font-size: 12px; }
.notes-box { background: #faf5ff; border: 1px solid #e9d5ff; border-radius: 8px; padding: 12px 16px; margin-bottom: 18px; }
.notes-box .lbl { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; color: #9333ea; margin-bottom: 6px; }
.notes-box p { font-size: 12px; color: #374151; white-space: pre-wrap; }
.sec-title { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .1em; color: #9333ea; border-bottom: 2px solid #e9d5ff; padding-bottom: 4px; margin-bottom: 10px; }
table { width: 100%; border-collapse: collapse; font-size: 12px; }
th { background: #f5f3ff; padding: 7px 10px; text-align: left; font-weight: 700; font-size: 9px; text-transform: uppercase; letter-spacing: .05em; color: #6d28d9; border-bottom: 2px solid #ddd6fe; }
td { padding: 7px 10px; border-bottom: 1px solid #f1f5f9; }
tr:last-child td { border-bottom: none; }
.tr-total { background: #f5f3ff; font-weight: 700; }
.tr-total td { border-top: 2px solid #ddd6fe; font-weight: 700; }
.num { text-align: right; }
.total-box { margin-top: 14px; display: flex; justify-content: flex-end; }
.total-inner { background: #faf5ff; border: 2px solid #d8b4fe; border-radius: 10px; padding: 12px 20px; display: flex; align-items: center; gap: 20px; }
.total-inner .lbl { font-size: 11px; color: #9333ea; font-weight: 600; text-transform: uppercase; letter-spacing: .05em; }
.total-inner .val { font-size: 22px; font-weight: 800; color: #7e22ce; }
...
<div class="header">
  ...
  <div class="sub">Club Balvanera · Hospitality</div>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>${fmtD(p.dia)}</td>
      <td>${p.turno ?? '—'}</td>
      <td class="num" style="font-weight:600;color:#7e22ce">${'$' + (p.compensacion ?? 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
    </tr>`).join('')}
    <tr class="tr-total">
      <td colspan="4" style="text-align:right;padding-right:10px;font-size:11px;color:#6d28d9;letter-spacing:.05em">TOTAL COMPENSACIONES</td>
      <td class="num" style="color:#7e22ce;font-size:14px">${'$' + totalComp.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
    </tr>
  </tbody>
</table>
<div class="firma">
  <div class="firma-line">Elaboró</div>
  <div class="firma-line">Hospitality</div>
          ...
          footer={(['info','operacion','checklist'] as string[]).includes(activeTab) ? (
            ...
            <>
              {editEvt && personal.length > 0 && (
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                  Total compensaciones: <strong style={{ color: '#9333ea' }}>{fmt$(personal.reduce((s, p) => s + (p.compensacion ?? 0), 0))}</strong>
                </div>
              )}
            </>
          ) : undefined}
        >
                      ...
                      <tbody>
                        {gastos.map((g, i) => (
                          <>
                            ...
                            {/* Inline OP search row para vincular */}
                            {vincGastoId === g.id && (
          ...
          {activeTab === 'personal' && editEvt && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="card" style={{ background: '#faf5ff', border: '1px solid #e9d5ff' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#9333ea', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>Justificación y Notas de Personal</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <button className="btn-primary" onClick={saveEvento} disabled={saving} style={{ fontSize: 12, background: '#9333ea' }}>
              ...
              <div className="card" style={{ background: '#f5f3ff', border: '1px solid #ddd6fe' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#9333ea', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>Agregar Empleado</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                  <div style={{ gridColumn: '1 / -1' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <button className="btn-primary" onClick={savePersonal} disabled={savingPersonal} style={{ fontSize: 12, background: '#9333ea' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  {personal.length > 0 && (
                    <button className="btn-ghost" onClick={printPersonal} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#9333ea', border: '1px solid #e9d5ff', borderRadius: 6, padding: '4px 10px' }}>
                      ...
                      <tbody>
                        {personal.map((p, i) => (
                          <tr key={p.id} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--surface-800)' }}>
                            ...
                            <td style={{ padding: '8px 10px', fontWeight: 600, color: '#9333ea' }}>{fmt$(p.compensacion ?? 0)}</td>
                            <td style={{ padding: '8px 10px' }}>
                        ...
                        <tr style={{ borderTop: '2px solid var(--border)', background: 'var(--surface-700)' }}>
                          ...
                          <td style={{ padding: '8px 10px', fontWeight: 700, color: '#9333ea' }}>{fmt$(personal.reduce((s, p) => s + (p.compensacion ?? 0), 0))}</td>
                          <td></td>
                        </tr>
                      </tbody>
                    </table>
