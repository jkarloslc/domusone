...
.notes-box { background: #f5f3ff; border: 1px solid #ddd6fe; border-radius: 8px; padding: 12px 16px; margin-bottom: 18px; }
.notes-box .lbl { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; color: #7c3aed; margin-bottom: 6px; }
.notes-box p { font-size: 12px; color: #374151; white-space: pre-wrap; }
.sec-title { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .1em; color: #7c3aed; border-bottom: 2px solid #ddd6fe; padding-bottom: 4px; margin-bottom: 10px; }
table { width: 100%; border-collapse: collapse; font-size: 12px; }
th { background: #f5f3ff; padding: 7px 10px; text-align: left; font-weight: 700; font-size: 9px; text-transform: uppercase; letter-spacing: .05em; color: #5b21b6; border-bottom: 2px solid #ddd6fe; }
td { padding: 7px 10px; border-bottom: 1px solid #f1f5f9; }
tr:last-child td { border-bottom: none; }
.tr-total { background: #f5f3ff; font-weight: 700; }
.tr-total td { border-top: 2px solid #ddd6fe; font-weight: 700; }
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>${fmtD(p.dia)}</td>
      <td>${p.turno ?? '—'}</td>
      <td class="num" style="font-weight:600;color:#7c3aed">${'$' + (p.compensacion ?? 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
    </tr>`).join('')}
    <tr class="tr-total">
      <td colspan="4" style="text-align:right;padding-right:10px;font-size:11px;color:#5b21b6;letter-spacing:.05em">TOTAL COMPENSACIONES</td>
      <td class="num" style="color:#7c3aed;font-size:14px">${'$' + totalComp.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
    </tr>
  </tbody>
</table>
<div class="firma">
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .monto-box { background: #f5f3ff; border: 2px solid #d8b4fe; border-radius: 10px; padding: 16px 20px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; }
  .monto-box .label { font-size: 12px; color: #7c3aed; font-weight: 600; }
  .monto-box .value { font-size: 28px; font-weight: 800; color: #5b21b6; }
...
.f label { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: #7c3aed; display: block; margin-bottom: 2px; }
...
.chk.done { background:#f5f3ff; border:1px solid #ddd6fe; }
...
<div class="sec">
  ...
  ${chkItems.map(([l, val]) => `<div class="chk ${val ? 'done' : 'pend'}"><span style="font-size:14px">${val ? '☑' : '☐'}</span><span>${l}</span>${val ? '<span style="margin-left:auto;font-size:10px;color:#5b21b6;font-weight:700">Completado</span>' : ''}</div>`).join('')}
</div>
<div class="sec">
  ...
  ${viewIng.length > 0 ? `<div class="tw"><table><thead><tr><th>Folio</th><th>Descripción</th><th>Forma de pago</th><th>Fecha</th><th style="text-align:right">Monto</th></tr></thead><tbody>${viewIng.map(i => `<tr><td style="font-family:monospace;color:#5b21b6">${i.folio}</td><td>${i.descripcion}</td><td>${i.forma_pago}</td><td>${new Date(i.fecha_pago + 'T12:00:00').toLocaleDateString('es-MX')}</td><td style="text-align:right;font-weight:600">$${i.monto.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td></tr>`).join('')}</tbody></table></div>` : ''}
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 20px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Total eventos', value: totalEventos, color: '#7c3aed', bg: '#f5f3ff' },
              ...
              {eventosFiltrados.map((ev, i) => {
                ...
                return (
                  <tr key={ev.id} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--surface-800)' }}>
                    <td style={{ padding: '9px 12px', fontWeight: 700, color: '#7c3aed', fontFamily: 'monospace', fontSize: 11 }}>{ev.folio}</td>
      ...
      {modal && (
        <ModalShell
          ...
          subtitulo={editEvt ? editEvt.status : 'Hípico — Ecuestres'}
          icono={HorseIcon}
          maxWidth={820}
          onClose={() => setModal(false)}
          tabs={[
            { key: 'info',      label: 'Información', icon: HorseIcon },
        ...
        >
          ...
          {activeTab === 'info' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#7c3aed', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>Datos del cliente</div>
          ...
          {activeTab === 'operacion' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {/* Ecuestre — sección principal */}
              <FmSection title="Datos del Evento Ecuestre" color="#7c3aed">
                <FmGrid2>
                  <div>
                    <label style={lblSt}>Tipo de evento ecuestre</label>
                    <input className="input" value={form.hip_tipo_evento} onChange={e => setForm(f => ({ ...f, hip_tipo_evento: e.target.value }))} placeholder="Concurso, Doma, Polo, Enduro…" style={inpSt} />
                  </div>
                  <div>
              ...
              {/* Resumen Ejecutivo */}
              <FmSection title="Resumen Ejecutivo" color="#5b21b6">
                <FmGrid2>
          ...
          {activeTab === 'checklist' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {/* Checklist operativo */}
              <FmSection title="Checklist Operativo" color="#5b21b6">
          ...
          {activeTab === 'ingresos' && editEvt && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="card" style={{ background: '#f5f3ff', border: '1px solid #ddd6fe' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#7c3aed', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>Registrar ingreso</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8, marginBottom: 10 }}>
                  ...
                  <button className="btn-primary" onClick={buscarVentasPOS} disabled={loadingVentasPOS} style={{ fontSize: 12, background: '#5b21b6' }}>
                    {loadingVentasPOS ? '…' : 'Buscar POS'}
                  </button>
                </div>

                {ventasPOS.length > 0 && (
                  <div style={{ maxHeight: 150, overflowY: 'auto', border: '1px solid #ddd6fe', borderRadius: 8, marginBottom: 10, background: '#fff' }}>
                    {ventasPOS.map(v => (
                      <button key={v.id}
                        onClick={() => seleccionarVentaPOS(v)}
                        style={{ width: '100%', textAlign: 'left', border: 'none', background: 'transparent', borderBottom: '1px solid #ede9fe', padding: '7px 10px', cursor: 'pointer' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                          <span style={{ fontSize: 12, fontWeight: 700, color: '#5b21b6', fontFamily: 'monospace' }}>{fmtFolioVentaPos(v)}</span>
                ...
                {ingresoForm.id_venta_pos_fk && (
                  <div style={{ marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', border: '1px solid #ddd6fe', borderRadius: 8, background: '#f5f3ff' }}>
                    ...
                    <button className="btn-ghost" onClick={() => setIngresoForm(f => ({ ...f, id_venta_pos_fk: null }))} style={{ fontSize: 11, color: '#4c1d95', padding: '2px 6px' }}>
                      Quitar
                    </button>
                  </div>
                )}
                ...
                <div style={{ marginTop: 12, display: 'flex', justifyContent: 'flex-end' }}>
                  <button className="btn-primary" onClick={saveIngreso} disabled={savingIngreso} style={{ fontSize: 12, background: '#7c3aed' }}>
                    ...
                    <tbody>
                      {ingresos.map((ing, i) => (
                        <tr key={ing.id} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--surface-800)' }}>
                          <td style={{ padding: '8px 10px', fontWeight: 700, color: '#7c3aed', fontFamily: 'monospace', fontSize: 10 }}>{ing.folio}</td>
                          <td style={{ padding: '8px 10px', color: '#4c1d95', fontSize: 11, fontFamily: ing.id_venta_pos_fk ? 'monospace' : 'inherit' }}>
                            {ing.id_venta_pos_fk
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  {personal.length > 0 && (
                    <button className="btn-ghost" onClick={printPersonal} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#7c3aed', border: '1px solid #ddd6fe', borderRadius: 6, padding: '4px 10px' }}>
          ...
          <ModalShell
            ...
            footer={
              <div style={{ display: 'flex', gap: 8, width: '100%', justifyContent: 'space-between', alignItems: 'center' }}>
                <button className="btn-primary" onClick={printFichaMaestra} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#5b21b6', fontSize: 13 }}>
