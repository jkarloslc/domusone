...
export default function AsignacionModal({ asignacion, idAsignacionFk, modoAgregarCuotas = false, montoExistente, onClose, onSaved }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    id_caballeriza_fk:  asignacion?.id_caballeriza_fk  ?? 0,
  ...
  useEffect(() => {
    Promise.all([
      ...
      dbHip.from('cat_caballerizas').select('id, clave, nombre, status').eq('activo', true).order('clave'),
      dbHip.from('cfg_hip').select('tarifa_mensual').single(),
    ]).then(([{ data: arr }, { data: cab }, { data: cfg }]) => {
      setArrendatarios((arr ?? []) as Arrendatario[])
      const cabs = (cab ?? []) as Caballeriza[]
      setCaballerizas(cabs)
      const t = (cfg as any)?.tarifa_mensual ?? 0
      setTarifa(t)
      if (!asignacion && !montoExistente) setForm(f => ({ ...f, monto_mensual: t }))
      setLoading(false)
    })
  }, [asignacion, montoExistente])
  ...
  const handleSave = async () => {
    ...
    if (esEdicion && asigId) {
      ...
      const { error } = await dbHip.from('ctrl_asignaciones').update(payload).eq('id', asigId)
    ...
    if (!soloCuotas) {
      const { data: newAsig, error: e1 } = await dbHip.from('ctrl_asignaciones').insert({
      ...
      // Actualizar estado de caballeriza a Rentada
      await dbHip.from('cat_caballerizas').update({ status: 'Rentada' }).eq('id', form.id_caballeriza_fk)
    }

    // 2. Generar cuotas en cxc_hip
    const cuotasInsert = cuotas.map(c => {
      ...
      return {
        ...
        concepto:           `Renta Caballeriza — ${c.label}`,
        periodo:            c.periodo,
    ...
    if (soloCuotas && asigId) {
      const { data: asigData } = await dbHip.from('ctrl_asignaciones')
        .select('id_arrendatario_fk, id_caballeriza_fk, monto_mensual').eq('id', asigId).single()
      if (asigData) {
        const ad = asigData as { id_arrendatario_fk: number; id_caballeriza_fk: number; monto_mensual: number }
    ...
    const { error: e2 } = await dbHip.from('cxc_hip').insert(cuotasInsert)
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {!soloCuotas && (
            <>
              ...
              {!esEdicion && (
                <div>
                  <label style={labelStyle}>Caballeriza *</label>
                  <select style={inputStyle} value={form.id_caballeriza_fk}
                    onChange={e => set('id_caballeriza_fk', Number(e.target.value))}>
                    <option value={0}>— Seleccionar —</option>
                    {caballerizas.map(c => (
                      <option key={c.id} value={c.id}
                        disabled={c.status === 'Rentada' || c.status === 'Bloqueada'}>
                        {c.clave}{c.nombre ? ` — ${c.nombre}` : ''} {c.status !== 'Libre' ? `(${c.status})` : ''}
                      </option>
                    ))}
                  </select>
                  {caballerizas.filter(c => c.status === 'Libre').length === 0 && (
                    <div style={{ fontSize: 11, color: '#dc2626', marginTop: 4 }}>Sin caballerizas disponibles.</div>
                  )}
                </div>
              )}
                  ...
                  {(['MENSUAL', 'ANUAL'] as const).map(m => (
                    <button key={m} onClick={() => setModalidad(m)}
                      style={{ flex: 1, padding: '9px', fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer',
                        background: modalidad === m ? '#fff7ed' : '#fff',
                        color: modalidad === m ? '#b45309' : '#94a3b8',
                ...
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 14 }}>
                  ...
                  <div style={{ borderTop: '1px solid #e2e8f0', marginTop: 8, paddingTop: 8, display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 700 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <span style={{ color: '#b45309' }}>${totalCuotas.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                </div>
              ) : (
